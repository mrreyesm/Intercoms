...
# 371 "./mcc_generated_files/eusart1.h"
void EUSART1_Transmit_ISR(void);
# 392 "./mcc_generated_files/eusart1.h"
...
# 467 "./mcc_generated_files/eusart1.h"
void EUSART1_SetErrorHandler(void (* interruptHandler)(void));
# 487 "./mcc_generated_files/eusart1.h"
void EUSART1_SetTxInterruptHandler(void (* interruptHandler)(void));
# 507 "./mcc_generated_files/eusart1.h"
...
void LCDport(char a)
{
 if(a & 1) LATAbits.LATA2 = 1;
 else LATAbits.LATA2 = 0;
 if(a & 2) LATAbits.LATA3 = 1;
 else LATAbits.LATA3 = 0;
 if(a & 4) LATAbits.LATA4 = 1;
 else LATAbits.LATA4 = 0;
 if(a & 8) LATAbits.LATA5 = 1;
...
void getInfo(void)
{
    ...
    LCDsc(2,1);
    LCDputs("    N:"); LCDcon();
    for(i = 0; i <= 7; i++){ keyboard[i] = '\0';};
    i = 1; cursor = 7;
    keyPress = KBDwaitDoor();
# 97 "main.c"
    if(keyPress == '#'){ LCDputc('*'); cursor++; verModeProg();}

    if(keyPress == '*'){ __asm("goto _getInfo"); }
    else{
        cursor++;
        keyboard[0] = keyPress;
        LCDputc(keyboard[0]);
...
void controlAcceso(int auxKey)
{
    ...
    if (auxKey == 0)
    {
        LCDclr(); LCDputs("  DEPARTAMENTO  "); LCDsc(2,1); LCDputs("  NO PERMITIDO  "); _delay((unsigned long)((800)*(16000000/4000.0)));
        __asm("goto _getInfo");
    }
...
void keyType(unsigned long apd)
{
    ...
    if (auxKey == 0)
    {
        LCDclr(); LCDputs("     CLAVE      "); LCDsc(2,1); LCDputs("   INCORRECTA   "); _delay((unsigned long)((800)*(16000000/4000.0)));
        __asm("goto _getInfo");
    }
...
void verModeProg(void)
{
    ...
    if(keyPress == '#')
    {
        ...
        do{
            ...
            if(keyboard[i] == '*') { __asm("goto _getInfo"); }
            LCDputc('*');
            i++; cursor++;
        } while((i <= 5));
...
void progCacc(void)
{
    ...
    ingresar();
        ...
        if (result == 0)
        {
            ...
            LCDputs("  NO PERMITIDO  ");
            _delay((unsigned long)((800)*(16000000/4000.0)));
            __asm("goto _progCacc");
        }
    cpto = 0;
    cpto = checkMem();
        unsigned long rd = 0, caux = 0;
...
endTest:
    ...
    _delay((unsigned long)((1800)*(16000000/4000.0)));
